...
| **NOT VERIFIED** | The code exists and is complete, but has never been executed. **This applies to every single file in this project.** |

---

## Verification status — read this first

> **NOT ONE LINE OF THIS PROJECT HAS EVER BEEN RUN.**
>
> MATLAB is not installed on the machine where it was written. Every `.m` file was written against documented MATLAB semantics, but none has been parsed by MATLAB, let alone executed.
>
> **Expect syntax errors and bugs on first run.** That is the normal and unavoidable consequence of writing several thousand lines without an interpreter. It is stated here plainly rather than discovered by a judge.
>
> The first task on the destination machine is `runAllTests('unit')`. Fix what fails. Record the real numbers.
...
|---|---|---|
| **Every `.m` file in this project** | MATLAB | Never executed |
...
| All test suites | MATLAB | **Never run. Do not report as passing.** |
| `python/crosscheck_frenet.py` | Python + a MATLAB-generated CSV | Never run — the CSV has never been produced |
